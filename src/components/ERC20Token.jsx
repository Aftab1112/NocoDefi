...
const ERC20Token = () => {
  ...
  return (
    <div className="flex justify-center items-center h-screen">
      {!isConfirmed && (
        <form className='flex flex-col gap-6 bg-gray-100 p-8 rounded-lg shadow-md' onSubmit={handleSubmit}>
          <input required className='bg-gray-200 rounded-md px-4 py-2 outline-none focus:bg-white' type="text" placeholder='Contract Name' onChange={handleContractNameChange} value={contractName} />
          ...
          <button disabled={isPending} className='bg-blue-500 text-white font-bold py-2 px-4 rounded-md hover:bg-blue-600 transition duration-300' type="submit">{isPending ? 'Confirming' : 'Submit'}</button>
        ...
        <div className='flex flex-col gap-6 bg-gray-100 p-8 rounded-lg shadow-md'>
          {isConfirmed && (
            <div className="bg-gray-200 rounded-md px-4 py-2">
              <span className="text-gray-700 font-bold">Transaction Confirmed</span> <br />
              <span className="text-gray-700">Previous Data:</span> <br />
              <span className="text-gray-700">Contract Name: {contractName}</span> <br />
              <span className="text-gray-700">Token Name: {tokenName}</span> <br />
          ...
          <form className='flex flex-col gap-6 mt-6' onSubmit={handleSubmit_next}>
            ...
            <button className='bg-blue-500 text-white font-bold py-2 px-4 rounded-md hover:bg-blue-600 transition duration-300' type="submit">Submit</button>
          </form>
        </div>
      )}
    </div>
